...
%    for yv = 1:7 % yv = y variable (columns)
%        for xv = 1:6 % xv = x variable (rows)
%            miniverticalCheck = column_full(game(xv, yv), game((xv+1), yv), game((xv+2), yv), game((xv+3), yv));
%            if miniverticalCheck == 3
%                verticalCheck = 3;
%                if game(xv, yv) == 1 || game(xv, yv) == 2
%                    player_error_announcement = title('This Column is full');
%                    set(player_error_announcement, 'color', 'w')
%                    set(player_error_announcement,'FontWeight','bold')
%                    set(player_error_announcement,'FontSize',19)
%                    break
 %               end
%                break
%            end
...
%        end
%    end
%two nested for loops that iterate to check for a vertical check from both
%players
%iterating through columns (y_variables) 1:7 and rows (x_variables) 1:3 looking for a match between columns where
...
elseif column_full == 3
    disp 'Error, column is full'
    break
end
...
%% function to check if column is full (called in line 62 for now)
function [full] = column_full(VerticalInput)
    if (HorizontalInput == 1 && VerticalInput == 1) || (HorizontalInput == 2 && VerticalInput == 2)
        full = 3
        return
    else
        full = 0
...
%the function check takes in two inputs which are two out of four
% different winning possibiliiters (rows, columns)
% and produces an output, full which is the end of the ability to play game
% %if q=3, column is full --> if q=0, column is not full. The if
% %statement above says that if any one of the two full conditions is
% %satisfied by either a one or a two (player one or player two), then the
% column is full. If they are not satisfied, it is not full and q=0
